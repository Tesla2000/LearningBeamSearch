...
if __name__ == "__main__":
    ...
    greedy_result = fmean(eval(Config.OUTPUT_RL_RESULTS.joinpath("Greedy").read_text()))
    for log_file in Config.MODEL_TRAIN_LOG.iterdir():
        ...
        del data
        plt.plot(x, y / greedy_result, label=labels_translator.get(log_file.name.partition('_')[0], log_file.name.partition('_')[0]))
    ...
    #     y_values = np.array(tuple(fmean(eval(Config.OUTPUT_RL_RESULTS.joinpath(f"{model_type}_{constraint}").read_text())) for constraint in Config.time_constraints))
    #     plt.plot(
    #         x_values,
    #         y_values / greedy_result,
