...
class RecurrentModel(BaseRegressor):
    ...
    def fill_state(self, working_time_matrix):
        self.states_by_size = list({} for _ in range(Config.n_tasks + 1))
        self.states_by_size[0][tuple()] = self.encoder(Tensor(working_time_matrix).unsqueeze(0)).flatten().unsqueeze(0)
        self._working_times = {}
        self._working_indexes = {}
    ...
    def forward(self, x):
        ...
        for index in absent_indexes:
            if (indexes := tuple(sorted(absent_indexes.copy() - {index}))) in self.states_by_size[len(absent_indexes.copy() - {index})]:
