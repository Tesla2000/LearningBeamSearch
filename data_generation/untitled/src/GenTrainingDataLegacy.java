...
public class GenTrainingDataLegacy extends DataGenerator{
    ...
    public static void main(String[] args) {
        ...
        while (true) {
            ...
            saveToTxtFile(new int[n_machines], flattenMatrix(workingTimeMatrix), bestNode.getValue(), "training_data_regression/" + workingTimeMatrix.length + "_" + n_machines + ".txt");
            for (int tasks = 0; tasks < n_tasks; tasks++) {
                ...
                if (timeMatrix.length == 0)
                    break;
                saveToTxtFile(bestNode.getState()[tasks], flattenMatrix(timeMatrix), bestNode.getValue(), "training_data/" + timeMatrix.length + "_" + n_machines + ".txt");
            }
        }
    }
}